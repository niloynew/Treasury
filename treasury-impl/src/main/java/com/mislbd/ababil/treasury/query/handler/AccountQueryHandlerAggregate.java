...
public class AccountQueryHandlerAggregate {
    ...
    public QueryResult<?> accountSearch(AccountQuery accountQuery){
        if(accountQuery.isAsPage()) {
            PagedResult<Account> accountPage = accountService.findAccounts(accountQuery.getPageable(),
                    accountQuery.getProductId(), accountQuery.getCurrencyCode(),
                    accountQuery.getOpenDate(), accountQuery.getExpiryDate(), accountQuery.getStatus());
            return QueryResult.of(accountPage);
        }
        else {
            List<Account> accounts =
                    accountService.findAccounts(accountQuery.getProductId(), accountQuery.getCurrencyCode(),
                            accountQuery.getOpenDate(), accountQuery.getExpiryDate(), accountQuery.getStatus());
